...
def create_ride_request(
    ...
    cur.execute(
        """
        INSERT INTO ride_requests (rider_id, area, target_time, status, driver_id, created_at)
        VALUES (?, ?, ?, ?, NULL, ?)
        """,
        (rider_id, area, target_time_iso, status, created_at),
    )
...
def find_candidate_drivers(
    ...
    Find drivers whose:
      - area matches
      - is_driver = 1
      - have a schedule on 'weekday' and 'direction'
      - depart_time between start_time and end_time (strings 'HH:MM')
    ...
    cur.execute(
        ...
        ORDER BY s.depart_time ASC, u.rating_avg DESC
        """,
