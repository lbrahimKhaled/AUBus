...
def handle_post_ride_request(msg: Message, context: Context) -> Message:
    ...
    payload = msg.get("payload", {})
    needed = ["area", "target_time", "direction"]
    ...
    area = payload["area"]
    target_time_iso = payload["target_time"]  # 'YYYY-MM-DDTHH:MM'
    direction = payload["direction"]
    ...
    try:
        datetime.fromisoformat(target_time_iso)
    except ValueError:
    ...
    try:
        candidates = find_candidates_for_request(
            area, target_time_iso, direction)
    except Exception:
